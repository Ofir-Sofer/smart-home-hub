...
bool test_parser_proccess_message() {
    MessageQueue<RawMessage> user_input_queue("user_input"); // user_input is a "device id" for the raw user inputs
    ...
    return poped_msg.m_device_id == device_id && poped_msg.m_cmd == cmd && poped_msg.m_user_id == expected_user_id && poped_msg.m_target == simple_encoder_default_target;
}
